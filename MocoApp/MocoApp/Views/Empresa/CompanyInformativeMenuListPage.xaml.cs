...
namespace MocoApp.Views.Empresa
{
    ...
    public partial class CompanyInformativeMenuListPage : ContentPage
    {
        Company Company;
        CompanyService service = new CompanyService();
        List<InformativeMenu> InformativeMenus;
        Location location;
        CheckinSub CheckinSub;
        string _categoryId;
        string seletedCategory;

        public CompanyInformativeMenuListPage(Company company, Location _location = null)
        {
            InitializeComponent();
        ...
        public void LoadMenus(int totalImages)
        {
            ...
            for (int i = 0; i < totalImages; i++)
            {
                ...
                lblName.Text = InformativeMenus[i].Name;
                lblName.StyleId = InformativeMenus[i].Id;
                ...
                if (i == 0 || _categoryId == InformativeMenus[i].Id)
                    lblFirst = lblName;

                stkCategory.Children.Add(lblName);
            }

            if (InformativeMenus.Count > 0)
            {
                ...
                {
                    _categoryId = InformativeMenus[0].Id;
                    DetailsRecognizer_Tapped(lblFirst, null);
                }
            }
        }
        ...
        public async void LoadItens()
        {
            ...
            var item = InformativeMenus.FirstOrDefault(x => x.Id == _categoryId);
